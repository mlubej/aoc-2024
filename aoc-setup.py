...
@click.command()
@click.option("--year", default="2024", type=str, help="Year of Advend of Code event.")
@click.option("--day", default=None, type=str, help="Day of Advent of Code event.")
def main(year, day):
    if day is None:
        day = str(dt.datetime.now().day)

    # create directory
    path = os.path.join(AOC_PATH, "days", f"day-{int(day):02d}")
    os.makedirs(path, exist_ok=True)

    # create templated notebook
    notebook_content = open(
        os.path.join(AOC_PATH, "notebook_template.ipynb"), "r"
    ).read()
